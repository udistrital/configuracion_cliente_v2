...
@Component({
  selector: 'ngx-list-metodo-http',
  templateUrl: './list-metodo_http.component.html',
  styleUrls: ['./list-metodo_http.component.scss'],
  })
export class ListMetodoHttpComponent implements OnInit {
  ...
  loadData(): void {
    this.configuracionService.get('metodo_http/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.configuracionService.delete('metodo_http/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'MetodoHttp deleted');
            }
         });
      }
    });
