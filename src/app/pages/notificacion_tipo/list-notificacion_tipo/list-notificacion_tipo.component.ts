...
@Component({
  selector: 'ngx-list-notificacion-tipo',
  templateUrl: './list-notificacion_tipo.component.html',
  styleUrls: ['./list-notificacion_tipo.component.scss'],
  })
export class ListNotificacionTipoComponent implements OnInit {
  ...
  loadData(): void {
    this.configuracionService.get('notificacion_tipo/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.configuracionService.delete('notificacion_tipo/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'NotificacionTipo deleted');
            }
         });
      }
    });
